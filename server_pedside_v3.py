...
def receive_ped_coords(client, userdata, msg):
    ...
    if (ped_id_mqtt in mqtt_to_cv_ids):
        ...
        try:
            json_data = json.dumps(obj_data, default=convert_to_serializable)
            hf.send(client=client, data=json_data, topic_send=sendbacktopic)
        except Exception as e:
            print(e)
            # TODO
            # There is a problem with the JSON Serialization, 
            # it said that it cannot serialize int64
            # convert_to_serializable this function seems to solve his problemed
        end1 = time.time()
